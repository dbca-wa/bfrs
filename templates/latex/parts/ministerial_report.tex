...
Records at {{report_date}} show Department of Biodiversity, Conservation and Attractions (DBCA) staff have attended {{ item_map.forest_total_all_tenure|floatformat:0|intcomma }} bushfires covering an area of {{ item_map.forest_total_area|floatformat:2|intcomma }} hectares in its three south-west forest regions for the period since 1 July {{ current_finyear }}. Of these, {{ item_map.forest_pw_tenure|floatformat:0|intcomma }} fires covering {{ item_map.forest_area_pw_tenure|floatformat:2|intcomma }} hectares occurred on lands where DBCA has a management interest.
\\~\\
Outside the south-west forest regions, records show {{ item_map.nonforest_total_all_tenure|floatformat:0|intcomma }} bushfires attended by DBCA staff have burnt through an area of {{ item_map.nonforest_total_area|floatformat:2|intcomma }} hectares.
\\~\\
...
\tabulinesep=1.0mm
\begin{longtabu} to \linewidth { | p{0.2\linewidth} | p{0.15\linewidth} | p{0.15\linewidth} | p{0.15\linewidth} | p{0.15\linewidth} | }
\hline
\textbf{Region} & \bfseries\makecell[c]{Total no. \\ DBCA Interest} & \bfseries\makecell[c]{Total area (ha) \\ DBCA Interest} & \bfseries\makecell[c]{Total no. \\ All tenure} & \bfseries\makecell[c]{Total area (ha) \\ All area} \\
\hline
\endhead
{% for row in rpt_map %}
  {% for region, data in row.items %}
    {% if 'Total' in region or 'TOTAL' in region %}
      \textbf{ {{ region }} } & \textbf{ {{ data.pw_tenure|floatformat:0|intcomma }} } & \textbf{ {{ data.area_pw_tenure|floatformat:2|intcomma }} } & \textbf{ {{ data.total_all_tenure|floatformat:0|intcomma }} } & \textbf{ {{ data.total_area|floatformat:2|intcomma }} } \\
    {% else %}
      {{ region }} & {{ data.pw_tenure|floatformat:0|intcomma }} & {{ data.area_pw_tenure|floatformat:2|intcomma }} & {{ data.total_all_tenure|floatformat:0|intcomma }} & {{ data.total_area|floatformat:2|intcomma }} \\
    {% endif %}
    \hline
  {% empty %}
...
\subsubsection{Stuart Smith \\DIRECTOR GENERAL   \\~\\ {{form.sig_date}} \hfill $\rule{5.5cm}{0.05mm}$ \\ \hfill MINISTER FOR ENVIRONMENT   \\ Att  \\~\\ \hfill Date:$\rule{4.5cm}{0.05mm}$}
\end{minipage}
