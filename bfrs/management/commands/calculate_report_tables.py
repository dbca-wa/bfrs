...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            csr.execute("INSERT INTO reporting_areaburnt (area, bushfire_id, tenure_id, region_id, has_fire_boundary)    SELECT ROUND((ST_Area(ST_Transform(ST_Intersection(bf.fire_boundary, sf.shape), 900914))/10000)::numeric,2), bf.id, 30, bf.region_id, True    FROM reporting_bushfire bf JOIN reporting_state_forest sf ON ST_Intersects(bf.fire_boundary, sf.shape)  WHERE fbr_fire_report_classification = 'State - Broadleaf' AND bf.report_status IN (3, 4) AND NOT bf.fire_not_found AND bf.id NOT IN (SELECT id FROM reporting_crossregion_fires);")
            print ("INSERT INTO reporting_areaburnt (area, bushfire_id, tenure_id, region_id, has_fire_boundary)    SELECT ROUND((ST_Area(ST_Transform(ST_Intersection(bf.fire_boundary, sf.shape), 900914))/10000)::numeric,2), bf.id, 30, bf.region_id, True    FROM reporting_bushfire bf JOIN reporting_state_forest sf ON ST_Intersects(bf.fire_boundary, sf.shape)  WHERE fbr_fire_report_classification = 'State - Broadleaf' AND bf.report_status IN (3, 4) AND NOT bf.fire_not_found AND bf.id NOT IN (SELECT id FROM reporting_crossregion_fires);")
            csr.execute("UPDATE reporting_areaburnt SET tenure_id = 30 WHERE id IN (SELECT ab.id FROM reporting_areaburnt ab JOIN reporting_bushfire bf ON ab.bushfire_id = bf.id  JOIN reporting_state_forest sf ON ST_Within(bf.origin_point, sf.shape) WHERE ab.tenure_id = 3 AND NOT has_fire_boundary AND sf.fbr_fire_report_classification = 'State - Broadleaf');")
            print ("UPDATE reporting_areaburnt SET tenure_id = 30 WHERE id IN (SELECT ab.id FROM reporting_areaburnt ab JOIN reporting_bushfire bf ON ab.bushfire_id = bf.id  JOIN reporting_state_forest sf ON ST_Within(bf.origin_point, sf.shape) WHERE ab.tenure_id = 3 AND NOT has_fire_boundary AND sf.fbr_fire_report_classification = 'State - Broadleaf');")
            ...
            csr.execute("UPDATE reporting_bushfire bf SET tenure_id = 30 WHERE bf.tenure_id = 3 AND bf.id IN (SELECT id FROM reporting_bushfire bf, reporting_state_forest sf WHERE ST_Within(bf.origin_point, sf.shape) AND sf.fbr_fire_report_classification = 'State - Broadleaf');")
            print ("UPDATE reporting_bushfire bf SET tenure_id = 30 WHERE bf.tenure_id = 3 AND bf.id IN (SELECT id FROM reporting_bushfire bf, reporting_state_forest sf WHERE ST_Within(bf.origin_point, sf.shape) AND sf.fbr_fire_report_classification = 'State - Broadleaf');")
            ...
            print ("INSERT INTO reporting_areaburnt (area, bushfire_id, tenure_id, region_id, has_fire_boundary) SELECT area, bushfire_id, 26, region_id, True FROM (SELECT SUM(ROUND((ST_Area(ST_Transform(ST_Intersection(ST_Intersection(bf.fire_boundary, sf.shape), r.geometry), 900914))/10000)::numeric, 2)) AS area,  bf.id as bushfire_id, r.id as region_id FROM reporting_bushfire bf JOIN reporting_state_forest sf ON ST_Intersects(bf.fire_boundary, sf.shape) JOIN bfrs_region r ON ST_Intersects(bf.fire_boundary, r.geometry) WHERE fbr_fire_report_classification = 'Native Hardwood' AND bf.id IN (SELECT id FROM reporting_crossregion_fires) and r.dbca GROUP BY bf.id, r.id) AS sqry;")
            print ("--State Forest (Coniferous) (4s)")
            ...
            print ("--State Forest (Broadleaf)")
            csr.execute("INSERT INTO reporting_areaburnt (area, bushfire_id, tenure_id, region_id, has_fire_boundary) SELECT area, bushfire_id, 30, region_id, True FROM (SELECT SUM(ROUND((ST_Area(ST_Transform(ST_Intersection(ST_Intersection(bf.fire_boundary, sf.shape), r.geometry), 900914))/10000)::numeric, 2)) AS area,  bf.id as bushfire_id, r.id as region_id FROM reporting_bushfire bf JOIN reporting_state_forest sf ON ST_Intersects(bf.fire_boundary, sf.shape) JOIN bfrs_region r ON ST_Intersects(bf.fire_boundary, r.geometry) WHERE fbr_fire_report_classification = 'State - Broadleaf' AND bf.id IN (SELECT id FROM reporting_crossregion_fires) and r.dbca GROUP BY bf.id, r.id) AS sqry;")
            print ("INSERT INTO reporting_areaburnt (area, bushfire_id, tenure_id, region_id, has_fire_boundary) SELECT area, bushfire_id, 30, region_id, True FROM (SELECT SUM(ROUND((ST_Area(ST_Transform(ST_Intersection(ST_Intersection(bf.fire_boundary, sf.shape), r.geometry), 900914))/10000)::numeric, 2)) AS area,  bf.id as bushfire_id, r.id as region_id FROM reporting_bushfire bf JOIN reporting_state_forest sf ON ST_Intersects(bf.fire_boundary, sf.shape) JOIN bfrs_region r ON ST_Intersects(bf.fire_boundary, r.geometry) WHERE fbr_fire_report_classification = 'State - Broadleaf' AND bf.id IN (SELECT id FROM reporting_crossregion_fires) and r.dbca GROUP BY bf.id, r.id) AS sqry;")
