...
class StateForestTenureTableUpdate:
    ...
    def __init__(self, settings, clean_up=False):
        ...
        self.table_name = "reporting_state_forest"
        self.clean_up = clean_up

    def run_sync(self):
        """Run the reporting_state_forest table update."""
        ...
        if not success_tables_script:
            logger.error("Failed to update reporting_state_forest table. Script execution failed.")
            return
        else:
            logger.info("reporting_state_forest table updated successfully.")
        ...
        if not success_populate:
            ...
            logger.info("reporting_state_forest table populated successfully from directory.")
    ...
    def run_tables_script(self):
        ...
        logger.info("Running reporting reporting_state_forest table update " + seen_datetime)
        ...
        try:
            ...
            csr.execute("CREATE INDEX idx_reporting_state_forest_type_"+db_suffix+" ON public.reporting_state_forest USING btree (fbr_fire_report_classification);")

            
            csr.execute("CREATE INDEX reporting_state_forest_shape_geom_idx_"+db_suffix+" ON public.reporting_state_forest USING gist (shape);")                        
            ...
            connection.commit()
            logger.info("reporting_state_forest table created from schema.")
            success = True
        except Exception as e:
            logger.error("Error creating reporting_state_forest table: {}".format(e))
            connection.rollback()
        finally:
            csr.close()
    ...
    def populate_from_directory(self):
        """Populate the reporting_state_forest table from GeoJSON files in a directory."""
        success = False
        try:
            ...
            logger.error("Error populating reporting_state_forest table from directory: {}".format(e))

        return success

    def populate_from_geojson_file(self, geojson_file):
        """Populate the reporting_state_forest table from a GeoJSON file."""
        geojson_data = None
        csr = connection.cursor()
        counter = 0
        try:
            ...
            for feature in geojson_data["features"]:
                ...
                fbr_plantation_classification = properties.get("FBR_PLANTATION_CLASSIFICATION", None)
                fbr_tenure_category = properties.get("FBR_TENURE_CATEGORY", None)
                fbr_fire_report_classification = properties.get("FBR_FIRE_REPORT_CLASSIFICATION", None)            
                ...
                """.format(
                    ...
                    (
                        fbr_plantation_classification,
                        fbr_tenure_category,
                        fbr_fire_report_classification,
                        shape_length,
                        shape_area,
                        pnt.json,
                    ),
            ...
            logger.error(
                "Error populating reporting_state_forest table from GeoJSON file {}: {}".format(
                    geojson_file, e
                )
            )
            connection.rollback()
