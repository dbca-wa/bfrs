...
class ReportingStateForestTenureExtractGeojson:

    def __init__(self, settings):
        self.auth_user = settings.KB_AUTH_USER
        self.auth_pass = settings.KB_AUTH_PASS
        self.KB_LAYER_URL = settings.KB_STATE_FOREST_LAYER_URL
        self.geojson_split_geometry_count = settings.GEOJSON_SPLIT_GEOMETRY_COUNT
        self.download_dir = settings.LAYER_DOWNLOAD_DIR+"/state_forest/"
    ...
    def run_sync(self):
        ...
        logger.info(f"Syncing State Forest enure KB Layer for BFRS {seen_datetime}")
        try:
...
def main():
    settings = {
        ...
        "KB_LAYER_URL": os.environ.get("KB_STATE_FOREST_LAYER_URL", None),
